...
def collect_APIdata(symbols: list[str], dates:list[str], interval:str):
    ...
    for symbol in symbols:
        ...
        data = response.json()
        df = pd.DataFrame(data, columns=['open_time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'quote_volume', 'nb_trades', 'ignore1', 'ignore2', 'ignore3'])
        df['dates'] = pd.to_datetime(df['open_time'], unit='ms')
        df = df[['open', 'high', 'low', 'close', 'volume']].set_index(df['dates'])
...
if __name__ == '__main__':
    user_data = collect_APIdata(['ETHBTC', 'BNBETH'], ['2023-01-01', '2024-01-01'], '1d')
    print(user_data)